...
- `.repos/effect-form` (revisado remotamente): motor existente para Effect v4/React 19 con `FormBuilder`, `FormReact.make`, Schema sync/async, cross-field errors, field atoms, `AsyncResult` de submit, dirty/touched, reset, arrays, `KeepAlive` y wizard. Su peer range (`effect`/`@effect/atom-react >= beta.52 < 4.0.1`) incluye la beta.100 actual.
