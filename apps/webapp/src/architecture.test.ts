...
describe("frontend architecture boundaries", () => {
  ...
  it("reads deployment configuration only at the composition boundary", () => {
    const readers = appFiles
      .filter(([, source]) => /import\.meta\.env\.(?:VITE_WEB_URL|VITE_ASSET_BASE_URL)/.test(source))
      .map(([path]) => path.replace(/^\.\//, ""))
    expect(readers).toEqual(["main.tsx"])
    for (const [path, source] of appFiles) {
      if (path.endsWith("/main.tsx")) continue
      expect(source, path).not.toMatch(/makeWebappConfig\(import\.meta\.env/)
    }
  })
})
