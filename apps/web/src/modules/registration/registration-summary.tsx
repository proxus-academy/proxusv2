...
import { Button, Card, CardContent, CardHeader, CardTitle, DescriptionList, Text } from "@proxus/ui"
import {
...
export function DraftSummary({ draft }: { readonly draft: RegistrationDraft }) {
  ...
  return (
    <Card aria-label={registration_summary_label()} marginTop="xl">
      <CardHeader><CardTitle>{registration_summary_title()}</CardTitle></CardHeader>
      <CardContent>
      <DescriptionList items={[
        { label: registration_summary_problem(), value: draft.problemOtherText ?? problem ?? registration_summary_pending(), action: <Button type="button" variant="ghost" onClick={() => edit("problem")}>{registration_summary_editProblem()}</Button> },
        { label: registration_summary_studies(), value: draft.path.map((node) => node.name).join(" → ") || registration_summary_pending(), action: <Button type="button" variant="ghost" onClick={() => edit("study")}>{registration_summary_editStudies()}</Button> },
        { label: registration_summary_profile(), value: draft.username === undefined ? registration_summary_pending() : `${draft.username}, ${String(draft.birthYear)}`, action: <Button type="button" variant="ghost" onClick={() => edit("profile")}>{registration_summary_editProfile()}</Button> },
      ]} />
      </CardContent>
    </Card>
  )
