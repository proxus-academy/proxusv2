...
describe("browser product locale", () => {
  ...
  it("keeps canonical replacement scoped, observable, and retryable without applying document state early", () => Effect.runPromise(Effect.gen(function*() {
    ...
    const location = makeObservableValue({ destination: initial, matches: [{ id: "root", params: {} }], search: "campaign=one" })
    ...
    const router: RouterService<Destination> = {
      ...
      replaceDestination: (next, options) => Effect.suspend(() => {
        ...
        current.set(next)
        location.set({ destination: next, matches: [{ id: "root", params: {} }], search })
        error.set(undefined)
