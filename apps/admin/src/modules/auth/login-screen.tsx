...
export function LoginScreen() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    const exit = await login(Schema.decodeUnknownSync(LoginWithPasswordInput)({ email, password }))
    if (Exit.isSuccess(exit)) await navigate({ to: "/admin/nodes", replace: true })
  }
