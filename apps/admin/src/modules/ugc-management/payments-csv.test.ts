...
const creator = new UgcUser({ id: creatorId, authUserId: null, userType: "creator", status: "creator", displayName: "Lucía, Creator", email: "lucia@proxus.test", countryCode: "ES", data: { _tag: "CreatorData", approvedAt: at, tierId: "base", profile: { tiktokHandle: null, instagramHandle: null, phone: null }, contracts: [], acquisition: { source: "inbound", outboundManagerId: null } }, version: 1, createdAt: at, updatedAt: at })
const campaign = new UgcCampaign({ id: campaignId, name: "Campaña", status: "finalized", startsAt: "2026-08-01T00:00:00.000Z", submissionsCloseAt: "2026-08-10T00:00:00.000Z", reconciliationEndsAt: "2026-08-17T00:00:00.000Z", data: { countries: ["ES"], formats: ["review"], tiers: [{ id: "base", label: "Base", videoTarget: 8, fixedAmountCents: 10_000 }], bonusRules: [], currency: "EUR", contractPolicy, requiredPlatforms: ["tiktok", "instagram"], managerIncentives }, version: 2, createdAt: at, updatedAt: at })
const payment = (status: "pending" | "paid", suffix: number) => new UgcPayment({ id: makeUgcPaymentId(`70000000-0000-4000-8000-${suffix.toString().padStart(12, "0")}`), recipientUserId: creatorId, relatedCreatorId: creatorId, campaignId, kind: "creator_campaign", sourceKey: `creator-campaign:${suffix}`, status, amountCents: 10_000, currency: "EUR", breakdown: { fixedAmountCents: 10_000, viewsBonusCents: 0, rankingBonusCents: 0, referralBonusCents: 0, manualAdjustmentCents: 0, adjustmentReason: null }, paidAt: status === "paid" ? at : null, createdAt: at, updatedAt: at })
