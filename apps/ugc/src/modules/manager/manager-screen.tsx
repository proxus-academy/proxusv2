...
function Pipeline({ workspace }: { readonly workspace: UgcWorkspace }) {
  ...
  return <div className="grid gap-5 lg:grid-cols-2"><Panel title="Añadir lead outbound" description="Solo puedes añadir países de tus mercados."><form className="grid gap-3 sm:grid-cols-2" onSubmit={createLead}><label className="ugc-field"><span>Nombre</span><input name="name" required /></label><label className="ugc-field"><span>Correo</span><input name="email" type="email" required /></label><label className="ugc-field"><span>País</span><input name="country" defaultValue="ES" maxLength={2} required /></label><label className="ugc-field"><span>Notas</span><input name="notes" /></label><button type="submit" className="ugc-action sm:col-span-2" disabled={waiting}><UserPlus className="mr-2 size-4" />Crear lead</button></form></Panel>
    ...
    <div className="lg:col-span-2"><Panel title={`Onboarding (${onboarding.length})`}><div className="grid gap-4 md:grid-cols-2">{onboarding.length === 0 ? <Empty /> : onboarding.map((creator) => <OnboardingCreator key={creator.id} creator={creator} workspace={workspace} />)}</div></Panel></div><div className="lg:col-span-2"><CommandError /></div>
  </div>
}

function OnboardingCreator({ creator, workspace }: { readonly creator: UgcUser; readonly workspace: UgcWorkspace }) {
  const run = useAtomSet(ugcCommandAction)
  const waiting = useCommandWaiting()
  ...
  return <article className="rounded-xl border p-4"><h3 className="font-bold">{creator.displayName}</h3><p className="text-sm text-slate-500">{creator.email} · {creator.data.missedMeetCount} ausencias</p>{!attended ? <p className="mt-3 text-sm font-semibold text-slate-500">Pendiente de completar la reunión</p> : creator.data.contract === null ? <form className="mt-4 grid gap-2" onSubmit={generate}><label className="ugc-field"><span>Documento</span><input name="document" required /></label><label className="ugc-field"><span>Domicilio</span><input name="address" required /></label><button type="submit" className="ugc-action" disabled={waiting}>Generar acuerdo de prueba</button></form> : <p className="mt-3 text-sm font-semibold">Acuerdo {creator.data.contract.signedAt === null ? "pendiente de firma" : "firmado"}</p>}{ready ? <button type="button" className="ugc-action mt-4" disabled={waiting} onClick={() => run(new StartTrial({ creatorId: creator.id }))}>Iniciar prueba con la configuración vigente</button> : null}</article>
}
...
function Creators({ workspace }: { readonly workspace: UgcWorkspace }) {
  ...
  return <div className="grid gap-5"><Panel title="Creadores y grupos"><div className="grid gap-4">{creators.length === 0 ? <Empty /> : creators.map((creator) => <article key={creator.id} className="rounded-xl border p-4"><h3 className="font-bold">{creator.displayName}</h3><p className="text-sm text-slate-500">{creator.countryCode} · {creator.status}</p>{creator.status === "trial" ? <div className="mt-3 flex flex-wrap gap-2"><button type="button" className="ugc-action" disabled={waiting} onClick={() => run(new EvaluateTrial({ creatorId: creator.id, outcome: "passed", tierId: "tier-1", reason: null }))}>Completa y supera</button><button type="button" className="ugc-action ugc-action-secondary" disabled={waiting} onClick={() => run(new EvaluateTrial({ creatorId: creator.id, outcome: "failed", tierId: null, reason: "Completa la prueba pero no alcanza los criterios" }))}>Completa, no supera</button><button type="button" className="ugc-action ugc-action-danger" disabled={waiting} onClick={() => run(new EvaluateTrial({ creatorId: creator.id, outcome: "incomplete", tierId: null, reason: "No completa los contenidos requeridos" }))}>No completa</button></div> : <Assign creator={creator} workspace={workspace} />}</article>)}</div></Panel><CommandError /></div>
}
