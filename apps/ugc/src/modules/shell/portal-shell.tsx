...
export function PortalShell({ session, children }: { readonly session: CurrentSession; readonly children: ReactNode }) {
  ...
  const navigation = manager ? [...baseNavigation, { to: "/ugc/manager" as const, label: "Gestión", icon: BriefcaseBusiness }] : baseNavigation
  ...
  return <div className="ugc-shell">
    <header className="sticky top-0 z-30 border-b border-black/[.07] bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex h-18 max-w-6xl items-center gap-5 px-4 sm:px-6">
        ...
        <nav aria-label="Principal" className="mx-auto hidden items-center rounded-full border border-black/[.08] bg-[#fafafa] p-1 md:flex">
    ...
    <nav aria-label="Principal móvil" className="fixed inset-x-0 bottom-0 z-30 grid border-t border-black/[.08] bg-white/95 px-2 pb-[env(safe-area-inset-bottom)] md:hidden" style={{ gridTemplateColumns: `repeat(${navigation.length}, minmax(0, 1fr))` }}>
