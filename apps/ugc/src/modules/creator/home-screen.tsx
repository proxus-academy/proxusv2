...
function Onboarding({ workspace, creator }: { readonly workspace: UgcWorkspace; readonly creator: UgcUser }) {
  ...
  const attended = workspace.meets.some((meet) => meet.creatorId === creator.id && meet.status === "attended")
  const preMeetComplete = creator.data.requirements.filter((requirement) => requirement.id === "profile" || requirement.id === "training").every((requirement) => requirement.completedAt !== null)
  const visibleRequirements = creator.data.requirements.filter((requirement) => attended || requirement.id === "profile" || requirement.id === "training")
  ...
  return <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
    <section><p className="text-sm font-bold text-[#793ef9]">ONBOARDING</p><h1 className="mt-2 text-3xl font-bold">{attended ? "Prepara tu periodo de prueba" : "Prepara tu reunión"}</h1><p className="mt-2 text-slate-500">{attended ? "Firma las condiciones y conecta las cuentas donde publicarás." : "Completa la introducción y elige una reunión de bienvenida."}</p>
      <div className="ugc-card mt-6 divide-y divide-slate-100 p-5">{visibleRequirements.map((requirement) => <div key={requirement.id} className="flex items-center gap-3 py-4 first:pt-0 last:pb-0"><span className={`grid size-7 place-items-center rounded-full ${requirement.completedAt === null ? "bg-slate-100 text-slate-400" : "bg-emerald-500 text-white"}`}>{requirement.completedAt === null ? "·" : <Check className="size-4" />}</span><span className="flex-1 font-semibold">{requirement.label}</span>{requirement.id === "training" && requirement.completedAt === null ? <button type="button" className="ugc-action ugc-action-secondary" disabled={waiting} onClick={() => run(new CompleteRequirement({ requirementId: "training" }))}>Completar</button> : null}</div>)}</div>
    </section>
    <aside className="space-y-4">
      {!attended && reserved !== undefined ? <div className="ugc-card p-5"><CalendarDays className="size-6 text-[#793ef9]" /><h2 className="mt-3 font-bold">Reunión reservada</h2><p className="mt-1 text-sm text-slate-500">{date(reserved.startsAt)} · {reserved.durationMinutes} min</p></div> : null}
      {!attended && reserved === undefined && preMeetComplete ? <div className="ugc-card p-5"><h2 className="font-bold">Elige una reunión</h2><div className="mt-3 grid gap-2">{available.length === 0 ? <p className="text-sm text-slate-500">No hay horarios disponibles ahora mismo.</p> : available.map((meet) => <button type="button" key={meet.id} className="rounded-xl border p-3 text-left text-sm font-semibold hover:border-[#793ef9]" onClick={() => run(new ReserveMeet({ meetId: meet.id }))}>{date(meet.startsAt)} · {meet.durationMinutes} min</button>)}</div></div> : null}
      {attended && creator.data.contract === null ? <div className="ugc-card p-5"><FileSignature className="size-6 text-[#793ef9]" /><h2 className="mt-3 font-bold">Preparando tu acuerdo</h2><p className="mt-1 text-sm text-slate-500">Tu manager lo generará con las condiciones vigentes del trial.</p></div> : attended && creator.data.contract?.signedAt === null ? <div className="ugc-card p-5"><h2 className="font-bold">Acuerdo listo para firmar</h2><pre className="mt-3 max-h-36 overflow-auto whitespace-pre-wrap rounded-xl bg-slate-50 p-3 text-xs">{creator.data.contract.renderedDocument}</pre><button type="button" className="ugc-action mt-4 w-full" disabled={waiting} onClick={() => run(new SignContract({}))}>Firmar acuerdo</button></div> : null}
      {attended && creator.data.requirements.find((item) => item.id === "social")?.completedAt === null ? <form className="ugc-card grid gap-3 p-5" onSubmit={social}><h2 className="font-bold">Cuentas de publicación</h2><label className="ugc-field"><span>TikTok</span><input name="tiktok" required /></label><label className="ugc-field"><span>Instagram</span><input name="instagram" required /></label><button type="submit" className="ugc-action" disabled={waiting}>Guardar cuentas</button></form> : null}
      <CommandError />
    </aside>
  </div>
}
...
function Trial({ workspace, creator }: { readonly workspace: UgcWorkspace; readonly creator: UgcUser }) {
  ...
  return <div><p className="text-sm font-bold text-[#793ef9]">PERIODO DE PRUEBA</p><h1 className="mt-2 text-3xl font-bold">{warming ? "Calienta tu cuenta" : ended ? "Tu prueba está en revisión" : "Publica tus vídeos de prueba"}</h1><p className="mt-2 text-slate-500">{warming ? `Podrás empezar el ${date(creator.data.publishingStartsAt)}.` : ended ? "Tu manager está revisando el resultado." : `${submitted} de ${creator.data.requiredVideoCount} vídeos registrados.`}</p><p className="mt-2 text-sm font-semibold text-slate-700">Compensación al completar: {(creator.data.completionCompensationCents / 100).toLocaleString("es-ES", { style: "currency", currency: creator.data.currency })}</p>{!warming && !ended ? <div className="mt-6 max-w-2xl"><VideoForm campaign={null} formats={creator.data.allowedFormats} /></div> : null}</div>
}

function CampaignHome({ workspace, creator }: { readonly workspace: UgcWorkspace; readonly creator: UgcUser }) {
  const run = useAtomSet(ugcCommandAction)
  const waiting = useCommandWaiting()
  if (creator.data._tag !== "CreatorData") return null
  ...
  const campaignVideos = workspace.videos.filter((video) => video.creatorId === creator.id && video.campaignId === active.id)
  const submitted = campaignVideos.length
  const metrics = campaignVideos.flatMap((video) => workspace.videoData.filter((item) => item.videoId === video.id).sort((left, right) => right.capturedAt.localeCompare(left.capturedAt)).slice(0, 1))
  const views = metrics.reduce((total, item) => total + item.tiktokViews + item.instagramViews, 0)
  const lastMetricsAt = [...metrics].sort((left, right) => right.capturedAt.localeCompare(left.capturedAt))[0]?.capturedAt
  const scheduled = now < Date.parse(active.startsAt)
  const reconciliation = now >= Date.parse(active.submissionsCloseAt)
  const pendingContract = creator.data.contracts.find((contract) => contract.campaignId === active.id && contract.signedAt === null)
  const awaitingContract = membership.status === "awaiting_contract"
  return <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
    <section><p className="text-sm font-bold text-[#793ef9]">{awaitingContract ? "NUEVAS CONDICIONES" : scheduled ? "PRÓXIMA CAMPAÑA" : reconciliation ? "CAMPAÑA EN REVISIÓN" : "CAMPAÑA ACTIVA"}</p><h1 className="mt-2 text-3xl font-bold">{active.name}</h1><p className="mt-2 text-slate-500">{awaitingContract ? "Esta campaña cambia condiciones materiales de tu último acuerdo. Revísalas y firma para confirmar tu plaza." : scheduled ? `Empieza el ${date(active.startsAt)}.` : reconciliation ? `Las publicaciones cerraron. Revisaremos los resultados hasta el ${date(active.reconciliationEndsAt)}.` : `Puedes registrar contenido hasta el ${date(active.submissionsCloseAt)}.`}</p>
      {awaitingContract && pendingContract !== undefined ? <div className="ugc-card mt-6 max-w-2xl p-5"><h2 className="font-bold">Condiciones pendientes de firma</h2><pre className="mt-3 max-h-44 overflow-auto whitespace-pre-wrap rounded-xl bg-slate-50 p-3 text-xs">{pendingContract.renderedDocument}</pre><button type="button" className="ugc-action mt-4" disabled={waiting} onClick={() => run(new SignContract({}))}>Firmar nuevas condiciones</button><CommandError /></div> : null}
      {!awaitingContract && !scheduled && !reconciliation ? <div className="mt-6 max-w-2xl"><VideoForm campaign={active} formats={active.data.formats} /></div> : null}
      {campaignVideos.length === 0 ? null : <div className="mt-6 max-w-2xl"><h2 className="text-lg font-bold">Vídeos de esta campaña</h2><div className="mt-3 grid gap-3">{[...campaignVideos].sort((left, right) => right.submittedAt.localeCompare(left.submittedAt)).slice(0, 3).map((video) => { const snapshot = workspace.videoData.filter((item) => item.videoId === video.id).sort((left, right) => right.capturedAt.localeCompare(left.capturedAt))[0]; return <article key={video.id} className="ugc-card p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-bold">{video.reference}</p><p className="mt-1 text-sm text-slate-500">{video.format} · {video.status}</p></div><p className="text-sm font-semibold">{((snapshot?.tiktokViews ?? 0) + (snapshot?.instagramViews ?? 0)).toLocaleString("es-ES")} views</p></div></article> })}</div></div>}
    </section>
    <aside className="ugc-card h-fit p-5"><h2 className="font-bold">Resumen</h2><dl className="mt-4 grid gap-4 text-sm"><div><dt className="text-slate-500">Progreso</dt><dd className="mt-1 font-bold">{submitted} / {tier?.videoTarget ?? "—"} vídeos</dd><dd className="text-xs text-slate-500">{tier === undefined ? "" : `${Math.max(0, tier.videoTarget - submitted)} restantes`}</dd></div><div><dt className="text-slate-500">Views registradas</dt><dd className="mt-1 font-bold">{views.toLocaleString("es-ES")}</dd><dd className="text-xs text-slate-500">{lastMetricsAt === undefined ? "Sin actualizar" : `Actualizadas ${date(lastMetricsAt)}`}</dd></div><div><dt className="text-slate-500">Tier</dt><dd className="mt-1 font-bold">{tier?.label ?? membership.tierId}</dd></div><div><dt className="text-slate-500">Pago base</dt><dd className="mt-1 font-bold">{tier === undefined ? "—" : (tier.fixedAmountCents / 100).toLocaleString("es-ES", { style: "currency", currency: active.data.currency })}</dd></div><div><dt className="text-slate-500">Formatos</dt><dd className="mt-1 font-bold">{active.data.formats.join(", ")}</dd></div></dl></aside>
  </div>
}
...
function VideoForm({ campaign, formats }: { readonly campaign: UgcCampaign | null; readonly formats: ReadonlyArray<string> }) {
  ...
  return <form className="ugc-card grid gap-4 p-5 sm:grid-cols-2" onSubmit={submit}><div className="sm:col-span-2 flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-[#f0ebff] text-[#793ef9]"><Upload className="size-5" /></span><div><h2 className="font-bold">Registrar vídeo</h2><p className="text-sm text-slate-500">Añade los enlaces publicados en TikTok e Instagram.</p></div></div><label className="ugc-field"><span>Formato</span><select name="format">{formats.map((format) => <option key={format}>{format}</option>)}</select></label><label className="ugc-field"><span>Referencia</span><input name="reference" required /></label><label className="ugc-field"><span>TikTok</span><input name="tiktokUrl" type="url" required /></label><label className="ugc-field"><span>Instagram</span><input name="instagramUrl" type="url" required /></label><div className="sm:col-span-2"><CommandError /><button type="submit" className="ugc-action mt-3" disabled={waiting}><Video className="mr-2 size-4" />Guardar vídeo</button></div></form>
}
