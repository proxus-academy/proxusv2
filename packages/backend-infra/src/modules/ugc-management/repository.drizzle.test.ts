...
const creator = new UgcUser({
  ...
  data: { _tag: "CreatorData", approvedAt: now, tierId: "base", profile: { tiktokHandle: "@creator", instagramHandle: null, phone: null }, contracts: [], acquisition: { source: "inbound", outboundManagerId: null } }, version: 1, createdAt: now, updatedAt: now,
})
const campaign = new UgcCampaign({
  ...
  reconciliationEndsAt: "2026-09-06T12:00:00.000Z", data: { countries: ["ES"], formats: ["testimonial"], tiers: [{ id: "base", label: "Base", videoTarget: 8, fixedAmountCents: 10_000 }], bonusRules: [], currency: "EUR", contractPolicy, requiredPlatforms: ["tiktok", "instagram"], managerIncentives },
...
describe("UgcRepository Drizzle contract", () => {
  test("round-trips every persisted UGC entity and detects stale aggregate writes", () => Effect.runPromise(
    withRepository(Effect.gen(function*() {
      ...
      yield* repository.memberships.insert(new UgcGroupMember({ id: makeUgcGroupMemberId(id(5)), groupId, creatorId, tierId: "base", status: "active", agreementTermsKey: "terms-v1", joinedAt: now, completedAt: null }))
      ...
      yield* repository.payments.insert(new UgcPayment({ id: makeUgcPaymentId(id(9)), recipientUserId: creatorId, relatedCreatorId: creatorId, campaignId, kind: "creator_campaign", sourceKey: "creator-campaign:test", status: "pending", amountCents: 10_000, currency: "EUR", breakdown: { fixedAmountCents: 10_000, viewsBonusCents: 0, rankingBonusCents: 0, referralBonusCents: 0, manualAdjustmentCents: 0, adjustmentReason: null }, paidAt: null, createdAt: now, updatedAt: now }))
      yield* repository.programConfigurations.insert(new UgcProgramConfiguration({ market: "ES", data: { trial: { durationDays: 14, warmingDays: 2, requiredVideoCount: 8, maxVideosPerDay: 2, minVideosPerWeek: 1, formats: ["testimonial"], requiredPlatforms: ["tiktok", "instagram"], completionCompensationCents: 7_200, currency: "EUR" }, contractPolicy, managerIncentives, historyRetentionDays: 90 }, version: 1, updatedAt: now }))
      ...
      expect(yield* repository.programConfigurations.list()).toHaveLength(1)
