...
describe("FormAtoms", () => {
  ...
  describe("uninitialized form access", () => {
    it("valueAtom read throws a descriptive error naming the field and Provider", () => {
      const runtime = Atom.runtime(Layer.empty)
      const form = makeTestForm()
      const atoms = FormAtoms.make({ runtime, formBuilder: form, onSubmit: () => {} })
      const registry = AtomRegistry.make()
      ...
      expect(() => registry.get(fieldAtoms.valueAtom)).toThrowError(/Provider/)
    })

    it("valueAtom write throws a descriptive error naming the field and Provider", () => {
      const runtime = Atom.runtime(Layer.empty)
      const form = makeTestForm()
      const atoms = FormAtoms.make({ runtime, formBuilder: form, onSubmit: () => {} })
      const registry = AtomRegistry.make()
      ...
      expect(() => registry.set(fieldAtoms.valueAtom, "Jane")).toThrowError(/Provider/)
    })

    it("initialValueAtom read throws a descriptive error naming the field and Provider", () => {
      const runtime = Atom.runtime(Layer.empty)
      const form = makeTestForm()
      const atoms = FormAtoms.make({ runtime, formBuilder: form, onSubmit: () => {} })
      const registry = AtomRegistry.make()
      ...
      expect(() => registry.get(fieldAtoms.initialValueAtom)).toThrowError(/Provider/)
    })

    it("touchedAtom read and write throw a descriptive error naming the field and Provider", () => {
      const runtime = Atom.runtime(Layer.empty)
      const form = makeTestForm()
      const atoms = FormAtoms.make({ runtime, formBuilder: form, onSubmit: () => {} })
      const registry = AtomRegistry.make()
      ...
      expect(() => registry.set(fieldAtoms.touchedAtom, true)).toThrowError(/Provider/)
    })
    ...
    it("submitAtom fails with a descriptive defect when submitted before initialization", async () => {
      ...
      expect((defect as Error).message).toContain("Provider")
    })
  })
